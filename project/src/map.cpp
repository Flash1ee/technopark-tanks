...
sf::Color ParseColor(const std::string &hexRGB) {
    ...
    if (*pEnd != '\0') {
        throw std::runtime_error(hexRGB +
                                 " is not valid hex-encoded RGB color");
    }
...
void MapObject::MoveBy(const sf::Vector2f &movement) {
    rect.left += movement.x;
    rect.top += movement.y;
    sprite.move(movement);
}

void MapObject::MoveTo(const sf::Vector2f &position) {
...
bool Level::LoadFromFile(const std::string &filepath) {
    ...
    // Element <map> should be root in  format.
    ...
    while (layerElement) {
        ...
        while (tileElement) {
            ...
            if (subRectToUse >= 0) {
                ...
                sprite.setPosition(static_cast<float>(x * m_tileWidth),
        ...
        while (objectGroupElement) {
            ...
            while (objectElement) {
                ...
                if (objectElement->Attribute("width") != nullptr) {
                    ...
                    const size_t index =
                        std::stoi(objectElement->Attribute("gid")) -
                        m_firstTileID;
...
MapObject Level::GetFirstObject(const std::string &name) const {
    ...
    for (size_t i = 0; i < m_objects.size(); i++)
        if (m_objects[i].name == name) return m_objects[i];
...
std::vector<MapObject> Level::GetAllObjects() const { return m_objects; }
